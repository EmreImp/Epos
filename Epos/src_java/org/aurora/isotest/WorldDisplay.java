...
public class WorldDisplay {
	...
	/*
	 * The coordinate is the top-left corner of the rectangle containing the
	 * diamond shape.
	 */
	...
	/*
	 * Pixel offset for drawing the top-left corner of the floor image (and all
	 * of its contents) relative to the top-left corner (0,0) of the JPanel.
	 */
	private int xOffset, yOffset;

	private ImagesLoader imsLoader;
	private IsoPanel atPanel;
	...
	/*
	 * WorldItems maintains a list of things that appear on the tile floor:
	 * blocks, pickups, and sprites, including where they are. The list is in
	 * back-to-front row order to make the drawing of the scene use the right
	 * z-ordering.
	 */
	...
	public WorldDisplay(ImagesLoader imsLd, IsoPanel atp) {
		imsLoader = imsLd;
		atPanel = atp;
		...
		/*
		 * The WorldItem object uses the floor info data for deciding where the
		 * world objects should be drawn in the JPanel.
		 */
		/*
		 * wItems = new WorldItems(tileWidth, tileHeight, evenRowX, evenRowY,
		 * oddRowX, oddRowY);
		 */
		initObstacles();
		...
		/*
		 * The world objects information is stored in the obstacles[][] array
		 * and the WorldItems object.
		 */
	} // end of WorldDisplay()
	...
	/*
	 * The format of the input lines are: image <name> // the name of the floor
	 * GIF numTiles x y // number of tiles in the x- and y- directions dimTile w
	 * h // the width and height of a single tile evenRow x y // start of first
	 * even row oddRow x y // start of first odd row and blank lines and comment
	 * lines.
	 */
	{
	...
	/*
	 * There are three kinds of world objects: no-gos (n), blocks (b), pickups
	 * (p), which are positioned at a given tile coordinate (x,y). The block and
	 * pickup names refer to their image filenames.
	 * 
	...
	/*
	 * Read multiple coordinate lines until the line contains a #: <x1>-<y1>
	 * (x2>-<y2> ..... .... #
	 */
	{
	...
	/*
	 * Format of a line: <x1>-<y1> (x2>-<y2> ..... [ # ] Each one is used tp set
	 * obstacles[x][y] to true
	 */
	{
	...
	// Token's format is <x>-<y>; return it as a Point object
	{
		...
		if (results.length != 2)
			...
			try {
				...
				System.out.println("Incorrect format for coordinates in "
						+ token);
			}
		}
	 ...
	 * b <blockName> <x1>-<y1> (x2>-<y2> ..... .... #
	 * 
	 * Each <blockName> will be stored with a unique name (<blockName> +
	 * blocksCounter) along with the image stored in Images/<blockName>.gif
	 */
	{
	...
	private boolean getBlocksLine(String line, String blockName,
			BufferedImage im)
	/*
	 * Format of a line: <x1>-<y1> (x2>-<y2> ..... [ # ]
	 * 
	 * Each coordinate is stored as a new entry in the WorldItems object and
	 * obstacles[x][y] is set to true.
	 */
	{
		StringTokenizer tokens = new StringTokenizer(line);
		String token;
		...
		while (tokens.hasMoreTokens()) {
			...
			// wItems.addItem( blockName+blocksCounter, BLOCK, coord.x, coord.y,
			// im);
	...
	/*
	 * Draw the world (with all its contents) to the screen. The WorldItems
	 * object already contains the blocks and pickups.
	 * 
	 * The sprites must be added, and then WorldItems' draw() method is called.
	 * The sprites are then removed, since they will probably move before the
	 * next draw, and will need to be added again in a new position.
	 */
	{
	...
	/*
	 * Is the tile location, pt, occupied by a pickup? findPickupName() will
	 * return null if there is no pickup there. This method is called by
	 * PlayerSprite
	 */
	{
	...
	/*
	 * Delete the pickup called name from WorldItems, and decrement the number
	 * of pickups. Tell AlienTilePanel that the game is over when the number of
	 * pickups drops to 0. This method is called by PlayerSprite.
	 */
	{
		/*
		 * if (wItems.removePickup(name)) { // try to remove it numPickups--; if
		 * (numPickups == 0) // player has picked up everything
		 * atPanel.gameOver(); } else
		 * System.out.println("Cannot delete unknown pickup: " + name);
		 */
	} // end of removePickup()

	public String getPickupsStatus()
	// IsoPanel uses this to update the screen stats
	{
	...
	/*
	 * public void addSprites(PlayerSprite ps, AlienSprite as[]) // add the
	 * player and aliens { player = ps; aliens = as; }
	 * 
	 * public Point getPlayerLoc() /* Used by the alien sprites to find the
	 * player's location. This means that WorldDisplay controls access to player
	 * info. * { return player.getTileLoc(); }
	 * 
	 * 
	 * public void hitByAlien() /* An alien tells WorldDisplay that the player
	 * has been hit. Pass this on to the player. * { player.hitByAlien(); }
	 * 
	 * 
	 * public void playerHasMoved(Point newPt, int moveQuad) /* The player calls
	 * this method to tell WorldDisplay that it has moved; tell the aliens and
	 * update the world display's offsets. * { for(int i=0; i < aliens.length;
	 * i++) aliens[i].playerHasMoved(newPt); // tell the aliens
	 * 
	 * updateOffsets(moveQuad); // update world's offset } // end of
	 * playerHasMoved()
	 */
