...
export class System extends galacticObject {
    constructor(aSeed) {
        ...
        let pair1 = 2 * ((this.theSeed[2] >> 8) & 31);
        this.tweakseed();
        let pair2 = 2 * ((this.theSeed[2] >> 8) & 31);
        this.tweakseed();
        let pair3 = 2 * ((this.theSeed[2] >> 8) & 31);
        this.tweakseed();
        let pair4 = 2 * ((this.theSeed[2] >> 8) & 31);
        this.tweakseed();
        var name = "";
        if (this.pairs[pair1] != '.')
            name += this.pairs[pair1];
        if (this.pairs[pair1 + 1] != '.')
            name += this.pairs[pair1 + 1];
        if (this.pairs[pair2] != '.')
            name += this.pairs[pair2];
        if (this.pairs[pair2 + 1] != '.')
            name += this.pairs[pair2 + 1];
        if (this.pairs[pair3] != '.')
            name += this.pairs[pair3];
        if (this.pairs[pair3 + 1] != '.')
            name += this.pairs[pair3 + 1];
        if (longnameflag > 0) /* bit 6 of ORIGINAL w0 flags a four-pair name */ {
            if (this.pairs[pair4] != '.')
                name += this.pairs[pair4];
            if (this.pairs[pair4 + 1] != '.')
                name += this.pairs[pair4 + 1];
        }
        this.theName = name;
